-- 010_create_chat_members.sql (safe reapply; only creates if missing)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname='public' AND tablename='chat_members' AND policyname='chat_members_select_own'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='chat_members' AND policyname='chat_members_insert_own'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='chat_members' AND policyname='chat_members_delete_own'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='chat_members' AND policyname='chat_members_update_creator'
  ) THEN
